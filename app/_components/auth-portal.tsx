...
type Member = { id: string; name: string; email: string; graduationYear: string; department: string; directoryConsent: boolean; role: MemberRole; createdAt: string };
...
export function LoginPortal() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setMessage("");
    try {
      const data = await requestJson<{ member: Member }>("/api/auth/login", { method: "POST", body: JSON.stringify({ email: form.get("email"), password: form.get("password") }) });
  ...
  return <div className="auth-card"><span className="auth-kicker">MEMBER LOGIN</span><h2>동문회원 로그인</h2><p>가입한 이메일과 비밀번호로 로그인해주세요.</p><form className="auth-form" onSubmit={submit}><label>이메일<input name="email" type="email" autoComplete="email" placeholder="name@example.com" required /></label><label>비밀번호<input name="password" type="password" autoComplete="current-password" placeholder="비밀번호를 입력하세요" required /></label><button type="submit" disabled={submitting}>{submitting ? "로그인 확인 중" : "로그인"}</button></form>{message && <AuthNotice tone="error">{message}</AuthNotice>}<p className="auth-link">아직 회원이 아니신가요? <Link href="/register">회원가입</Link></p></div>;
}
...
export function RegisterPortal() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setMessage("");
    try {
      await requestJson<{ member: Member }>("/api/auth/register", { method: "POST", body: JSON.stringify({ name: form.get("name"), email: form.get("email"), password, graduationYear: form.get("graduationYear"), department: form.get("department"), directoryConsent: Boolean(form.get("directoryConsent")) }) });
...
export function MyPagePortal() {
  ...
  return <div className="auth-card account-card"><span className="auth-kicker">MY PAGE</span><h2>{member.name} 님, 반갑습니다.</h2><p>등록한 동문회원 정보를 확인할 수 있습니다.</p><dl className="account-details"><div><dt>이메일</dt><dd>{member.email}</dd></div><div><dt>졸업기수</dt><dd>{member.graduationYear}</dd></div><div><dt>졸업 학과</dt><dd>{member.department}</dd></div><div><dt>동문 찾기 공개</dt><dd>{member.directoryConsent ? "공개" : "비공개"}</dd></div></dl><div className="account-actions">{member.role === "admin" && <Link href="/admin">관리자 페이지</Link>}<button type="button" onClick={logout}>로그아웃</button></div></div>;
}
...
export function AdminPortal() {
  ...
  const [memberForm, setMemberForm] = useState({ name: "", email: "", password: "", graduationYear: "관리자 등록", department: "", role: "member" as MemberRole });
  const [message, setMessage] = useState("");
